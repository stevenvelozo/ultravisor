...
How to ship `ultravisor` to npm and to GitHub Container Registry
(GHCR). Generated by `quack docker-init`; the structure matches the
shared template across all dockerized retold tools.

`ultravisor` is a **long-running service**. Restart policy in compose / k8s should be `unless-stopped` (or equivalent). The Dockerfile should declare a HEALTHCHECK against the service's health endpoint.
...
- **Git remote configured** — `git remote get-url origin` should print
  `git@github.com:stevenvelozo/ultravisor.git` (or the
  HTTPS equivalent).
...
BUILD_DOCKER=1 npm publish    (or: npm run publish:docker)
  ...
  → docker push ghcr.io/stevenvelozo/ultravisor:<version>
```
...
1. **npm**: `npm view ultravisor version`
2. **Workflow**: `https://github.com/stevenvelozo/ultravisor/actions`
3. **Image**: `docker pull ghcr.io/stevenvelozo/ultravisor:latest`
...
```bash
docker pull ghcr.io/stevenvelozo/ultravisor:latest
docker run --rm ghcr.io/stevenvelozo/ultravisor:latest
```
